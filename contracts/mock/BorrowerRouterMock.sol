...
contract BorrowerRouterMock is IBorrowerRouter, Initializable {
    ...
    {
        return
            convertToAssetTokens(_assetAddr, _vaultTokenAddr, getCurrentInterest(_vaultTokenAddr))
                .convertTo18(ERC20(_assetAddr).decimals());
    }
    ...
    function set3CrvAddr(address _new3CrvAddr) external {
        pool3Crv = _new3CrvAddr;
    }
    ...
    function _crvMetaPoolDeposit(
        ...
        require(
            IMetaPool(_poolAddr).base_pool() == pool3Crv,
            "BorrowerRouter: Unsupported meta pool."
        );

        (, uint256 _indexInPool) = getPoolInfo(_crvRegistry, _poolAddr, _assetAddr, true);
    ...
    function _crvBasePoolDeposit(
        ...
        (uint256 _numberOfCoins, uint256 _indexInArray) = getPoolInfo(
            _crvRegistry,
            _poolAddr,
            _assetAddr,
            false
    ...
    function _yEarnWithdraw(
        ...
        if (!_isMaxWithdraw && _withdrawAmountInToken < _assetDepositAmountInToken) {
            ...
            _withdrawAmountInToken =
                ...
                getCurrentInterest(_vaultTokenAddr);
        }
    ...
    function _crvBasePoolWithdraw(
        ...
        (uint256 _numberOfCoins, uint256 _indexInPool) = getPoolInfo(
            _crvRegistry,
            _poolAddr,
            _assetAddr,
            false
    ...
    function convertToAssetTokens(
        ...
        (address _poolAddr, bool _isMeta) = _getCurvePool(_crvRegistry, _vaultTokenAddr);
        (, uint256 _indexInPool) = getPoolInfo(_crvRegistry, _poolAddr, _assetAddr, _isMeta);
    ...
    function getCurrentInterest(address _vaultTokenAddr) public view returns (uint256) {
        IYearnVault _vault = IYearnVault(vaultsDepoitInfo[_vaultTokenAddr].vaultAddr);
