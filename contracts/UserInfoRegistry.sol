...
contract UserInfoRegistry is IUserInfoRegistry, AbstractDependant {
    ...
    mapping(address => StatsForPRT) internal _PRTuserStats;
    ...
    function updateUserStatsForPRT(
        ...
        if (repaysCount_ > 0) {
            _PRTuserStats[userAddr_].repaysNum += repaysCount_;
        }

        if (liquidationsCount_ > 0) {
            _PRTuserStats[userAddr_].liquidationsNum += liquidationsCount_;
        }

        _updateUserPositionStatsForPRT(
            userAddr_,
            isSupply_
                ? _PRTuserStats[userAddr_].supplyStats
                : _PRTuserStats[userAddr_].borrowStats,
            isSupply_
    ...
    function getUserPRTStats(
        ...
        return _PRTuserStats[userAddr_];
    }
